...
def init_run(rank: int, world_size: int, cfg: IndexConfig, ds: Dataset | IterableDataset):
    ...
    return {"model": model, "data": ds, "processor": processor, "target_modules": target_modules}


def apply_ekfac_worker(rank: int, world_size: int, cfg: IndexConfig, ds: Dataset | IterableDataset):
    run_dict = init_run(rank, world_size, cfg, ds)
    model = run_dict["model"]
    ds = run_dict["data"]
    processor = run_dict["processor"]
    target_modules = run_dict["target_modules"]
...
def apply_ekfac_distributed(cfg: IndexConfig):
    distributed_computing(cfg=cfg, worker_fn=apply_ekfac_worker)
